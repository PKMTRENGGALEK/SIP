    ...
    .then((html) => {
      ...
      switch (page) {
        ...
        case "rtm.html":
          ...
          break;
        case "timmutu.html":
          waitForElement("#mututable", inittimmutu);
          break;
...
//tampil link dokumen tim mutu
async function inittimmutu() {
  const scriptURL =
    "https://script.google.com/macros/s/AKfycbzFW8gW5HjTxwOuTaaC8S1xhWYPyvo_MpLYVqfEwAa-4hUbDqRcBrqeinyd2ygXFqbhCg/exec";
  ...
  function renderTable(data) {
    if ($.fn.DataTable.isDataTable("#mututable")) {
      $("#mututable").DataTable().destroy();
    }
    ...
    <tr>
      ...
      <td class="text-center" style="font-size: 15px;">${nomor}.</td>
      <td class="text-start" style="font-size: 15px;">${row["Name"] || ""}</td>
      <td class="text-center" style="font-size: 15px;">${linkHTML}</td>
      <td class="text-center" style="font-size: 15px;">${actionButtons}</td>
    ...
    $("#mututable tbody").html(tableBody);
    $("#mututable").DataTable();
  }
  ...
  // Loading
  function showLoading() {
    const colCount = $("#mututable thead th").length;
    $("#mututable tbody").html(`
    <tr>
      <td colspan="${colCount}" class="text-center">
        <div class="spinner-border spinner-border-sm text-primary me-1" role="status"></div>
        Memuat data...
